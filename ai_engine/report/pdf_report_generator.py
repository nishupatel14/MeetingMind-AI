...
class PDFReportGenerator:
    ...
    def generate_direct(self, pdf_name="meeting_report.pdf", topics_data=None, decisions_data=None, actions_data=None):
        """Directly build a PDF from in-memory topics, decisions, and actions."""
        PDF_REPORT_FOLDER.mkdir(parents=True, exist_ok=True)
        output = PDF_REPORT_FOLDER / pdf_name

        topics_data = topics_data or []
        decisions_data = decisions_data or []
        ...
        pill_table = Table(
            [[
                Paragraph("<b>Generated by:</b> MeetingMind AI", self.pill_left_style),
                Paragraph("<b>Status:</b> Confirmed", self.pill_right_style),
            ]],
            colWidths=[270, 270]
        )
        ...
        # 1. Main Topics Section
        if topics_data:
            story.append(self.make_section_header("Main Discussion Topics"))
            story.append(Spacer(1, 6))
            for idx, top in enumerate(topics_data[:4], start=1):
                if isinstance(top, dict):
                    title = top.get("title", "")
                    details = top.get("details", [])
                    story.append(Paragraph(f"<b>{idx}. {html.escape(title)}</b>", self.sub_heading_style))
                    for d in details[:1]:
                        story.append(Paragraph(f"&bull; &nbsp; {html.escape(d)}", self.body_style))
                else:
                    story.append(Paragraph(f"&bull; &nbsp; {html.escape(str(top))}", self.body_style))
                story.append(Spacer(1, 4))
            story.append(Spacer(1, 4))
        ...
        if decisions_data:
            story.append(self.make_section_header("Confirmed Decisions"))
            story.append(Spacer(1, 6))
            for idx, dec in enumerate(decisions_data[:5], start=1):
                if isinstance(dec, dict):
                    d_txt = dec.get("decision", "")
                    d_ctx = dec.get("context", "")
                    story.append(Paragraph(f"<b>{idx}. {html.escape(d_txt)}</b>", self.sub_heading_style))
                    if d_ctx:
                        story.append(Paragraph(f"&bull; Context: {html.escape(d_ctx)}", self.body_style))
                else:
                    story.append(Paragraph(f"&bull; &nbsp; {html.escape(str(dec))}", self.body_style))
                story.append(Spacer(1, 4))
            story.append(Spacer(1, 4))
            story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#CBD5E1"), spaceAfter=10, spaceBefore=2))
        ...
        if actions_data:
            ...
            story.append(Spacer(1, 6))
            for idx, act in enumerate(actions_data[:6], start=1):
